...
const DonationSection: React.FC = () => {
  ...
  const [donation, setDonation] = useState<string>("ISKCON Rama Navami Anna Daan"); // State for donation
  ...
  const openModalWithPriceMahaKumbh = (price: string) => {
    setSelectedPrice(price); // Set the selected meal price
    setDonation("ISKCON Rama Navami Anna Daan");
    setIsModalOpen(true); // Open the modal
  };

  const openModalWithAnyPriceMahaKumbh = () => {
    if(selectedPrice)
    {
      setDonation("ISKCON Rama Navami Anna Daan");
      setIsModalOpen(true);
  ...
  const openModalWithPriceMakara = (price: string) => {
    setSelectedPrice(price); // Set the selected meal price
    setDonation("ISKCON Rama Navami Anna Daan");
    setIsModalOpen(true); // Open the modal
  };

  const openModalWithAnyPriceMakara = () => {
    if(selectedPrice)
    {
      setDonation("ISKCON Rama Navami Anna Daan");
      setIsModalOpen(true);
  ...
  return (
    <section className="donation-section">
      ...
      <h1 className="donation-title">ISKCON Rama Navami</h1>
      ...
      {/* Rama Navami */}
      <h2 className="meal-donation-title">Donate for Rama Navami Anna Daan</h2>
      <div className="meal-options">
        {[
          ...
          <div key={index} className="meal-option">
            ...
            <button className="add-donation-button" onClick={() => openModalWithPriceMakara(meal.price)} >Add Donation</button>
          </div>
        ))}
  ...
  <h3 style={{marginBottom:'10px'}}>Any Donation of Your Choice for Rama Navami Anna Daan</h3>
  ...
  <button style={{backgroundColor: '#faa817',color: '#fff',border: 'none',padding: '10px 15px',borderRadius: '5px',cursor: 'pointer',height:'63px',fontSize:'25px',marginTop:'10px'}}  onClick={() => openModalWithAnyPriceMakara()}>Donate</button>
</div>
...
<div className="account-details">
        <div className="account-info">
          ...
          <p>Account Name: ISKCON Rama Navami Fund  </p>
      ...
      <div className="meal-options">
        {[
          ...
          <div key={index} className="meal-option">
            <p className="meal-label">{meal.label}</p>
            <p className="meal-price">₹{meal.price}</p>
            <button className="add-donation-button" onClick={() => openModalWithPriceMahaKumbh(meal.price)} >Add Donation</button>
          </div>
        ))}
...
<DonateModal
        ...
        donation={donation} // Pass donation to modal
      />
...
<div className="custome-donation" style={{background:'white',borderRadius:'10px',padding:'50px',marginTop:'10px'}}>
  ...
  <input type="number" style={{width:'70%',borderRadius:'5px',marginRight:'10px',border:'1px solid',height:'50px',fontSize:'25px',fontWeight:'500',padding:'5px',marginTop:'10px'}} value={selectedPrice} onChange={handleInputChange} placeholder="Enter the Amount"></input>
  <button style={{backgroundColor: '#faa817',color: '#fff',border: 'none',padding: '10px 15px',borderRadius: '5px',cursor: 'pointer',height:'63px',fontSize:'25px',marginTop:'10px'}}  onClick={() => openModalWithAnyPriceMahaKumbh()}>Donate</button>
</div>
